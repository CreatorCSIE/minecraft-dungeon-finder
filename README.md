...
工具内置 4 个预置版本，覆盖不同版本的「箱子 loot RNG 消费」实现（红石 / 唱片 / 可可豆的加入会改变 RNG 消费顺序，从而影响刷怪笼怪物 ID）：

| 预置版本 | 覆盖范围 |
|---------|---------|
| `inf-20100625-1917` | `inf-20100625-1917` ~ `inf-20100630-1835` |
| `a1.0.1` | `a1.0.1` ~ `a1.0.13_01` |
| `a1.0.14` | `a1.0.14` ~ `b1.3_01` |
| `b1.4` | `b1.4` ~ `1.4.5`（不保证 `1.0.0` 以上可行） |

> 说明：由于版本间箱子 loot 的 RNG 消费实现不同，需按目标版本选择对应预置。`b1.4` 项覆盖 `1.4.5` 仅是实验性支持，`1.0.0` 之后的地牢生成可能有额外变化，不保证结果一致。
